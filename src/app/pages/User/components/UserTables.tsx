...
import { UserItems } from '../slice/types';
import { PagginationType } from 'utils/types/const';
import { ConfirmDeleteModal } from 'app/components/ConfirmDeleteModal';
import QueryString from 'qs';
import { userDelete } from 'services/userApi';
...
export function UserTables({
  ...
  const rowSelection: TableRowSelection<UserItems> = {
    selectedRowKeys,
        ...
        if (key === 'username') {
          columnView.render = (_, record, index) =>
            renderConfig['system'](_, record[key], index, () =>
              navigate(`${corepulseRoot}/user/detail?id=${record.id}`),
            );
        }
        return columnView;
      }),
          ...
          <ConfirmDeleteModal
            ...
            action={async () => {
              try {
                ...
                if (data) {
                  dispatch(usersActions.deleteUsers([record.id]));
                  getMessage(data);
                }
              } catch ({ response }: any) {
